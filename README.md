...
`detect` コマンドを使うと、Mac上で既に動いているClaude CLIプロセスを検出し、Slackスレッドから操作できるようになります。検出されたインスタンスは `NOTIFICATION_CHANNEL` にスレッドとして登録されます。
...
### bind fork: 実行中のCLIプロセスをチャンネルにフォーク

`detect` はNOTIFICATION_CHANNELにインスタンスを登録しますが、Project/Sessionモデルの外で動きます。`bind fork` を使うと、実行中のClaude CLIプロセスを **コマンド実行チャンネルのProject+Session** として統合できます。
